import type { KrakenDailyKlineRecord } from '@coqui/adapters';
import { instrumentKey } from '@coqui/core';
import { upsertMarketBars, type Db } from '@coqui/storage';

/** Persist verified Kraken research rows without erasing exact decimal text. */
export function persistKrakenDailyKlines(
  ...
  for (const record of records) {
    if (record.instrument.venue !== 'kraken' || record.instrument.productType !== 'spot') {
      throw new TypeError('Kraken archive rows require Kraken spot identities.');
    }
