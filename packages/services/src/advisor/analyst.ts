...
export type AnalystProvider = AdvisorProviderName;
...
const PROVIDER_KEYS: Readonly<Record<AnalystProvider, SecretKey>> = {
...
export class AdvisorAnalystService {
  ...
  constructor(private readonly input: { readonly profileId: string; readonly database: Db;
    readonly clock: Clock; readonly providers: Readonly<Record<AnalystProvider, AdvisorProvider>>;
    readonly secrets: SecretStore }) {}

  async providers() {
    return { providers: await Promise.all((Object.keys(PROVIDER_KEYS) as AnalystProvider[]).map(async (provider) => {
      const secret = await this.input.secrets.read(PROVIDER_KEYS[provider], this.input.profileId);
      return { provider, credentialState: !secret.ok ? 'unavailable' as const : secret.value === null ? 'disconnected' as const : 'connected' as const, model: this.input.providers[provider].model };
    })) };
  }
  ...
  async generate(contextHash: string, provider: AnalystProvider | null) {
    ...
    try {
      ...
      return this.#answer(context, provider, this.input.providers[provider].model, 'analysis', text);
    } catch {
      ...
      return this.#answer(context, 'local', 'deterministic-v1', 'analysis',
        context.localFacts.map((fact) => `${fact.label}: ${fact.value}`).join('\n'));
    }
  }
  ...
  async send(value: { readonly contextHash: string; readonly provider: AnalystProvider; readonly mode: AnalystMode; readonly message: string; readonly conversationId: string | null; readonly retention: 'session' | 'encrypted' }) {
    ...
    let answerText: string, answerProvider: AnalystProvider | 'local' = value.provider;
    ...
    try { answerText = await this.#request(value.provider, value.mode, value.message, context); }
      ...
      answerText = context.localFacts.map((fact) => `${fact.label}: ${fact.value}`).join('\n');
      answerProvider = 'local';
      providerSucceeded = false;
    }
    const id = value.conversationId ?? randomUUID(), now = this.input.clock.nowMs();
    const answer = this.#answer(context, answerProvider,
      answerProvider === 'local' ? 'deterministic-v1' : this.input.providers[value.provider].model,
      value.mode, answerText);
    ...
    if (value.retention === 'encrypted') await this.#persist(id, value.message.slice(0, 80), messages,
      answerProvider === 'local' ? null : value.provider, context.contextHash, now);
    else {
    ...
    if (providerSucceeded) appendAdvisorAuditEvent(this.input.profileId, value.provider, 'chat', 'succeeded', value.contextHash, now, this.input.database);
  ...
  async #request(provider: AnalystProvider, mode: AnalystMode, question: string, context: PreparedContext): Promise<string> {
    ...
    const adapter = this.input.providers[provider], secret = await this.input.secrets.read(PROVIDER_KEYS[provider], this.input.profileId);
    ...
    return adapter.generate({ apiKey: secret.value, system,
      evidenceJson: context.providerSummary, question });
  }
  ...
  async #persist(conversationId: string, title: string, messages: readonly MessageView[], provider: AnalystProvider | null, contextHash: string, now: number): Promise<void> {
    ...
    messages.forEach((message, index) => saveAdvisorMessage({ id: message.id, conversationId,
      sequence: sequence + index, role: message.role, provider: message.role === 'assistant' ? provider : null,
      modelPolicy: message.role === 'assistant' && provider !== null ? this.input.providers[provider].model : null,
      contextHash, ...encrypt(key, message.text), createdAtMs: message.createdAtMs }, this.input.database));
  }
}
