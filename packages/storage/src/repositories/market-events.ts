...
export function listMarketEventsAsOf(profileId: string, asOfMs: number, limit: number,
  ...
  const rows = database.prepare(`SELECT event.content_json,event.content_hash,event.provenance_hash,
    ...
    ORDER BY event.published_at DESC,event.id DESC LIMIT ?`).all(asOfMs, profileId, asOfMs, limit) as unknown as Array<{
      content_json: string; content_hash: string; provenance_hash: string;
      classification_id: string | null; result_json: string | null; result_hash: string | null;
    }>;
  return Object.freeze(rows.map((row) => {
    const event = JSON.parse(row.content_json) as MarketEventV1;
    assertEvent(event);
    if (marketEventHash(event) !== row.content_hash || sha256Hex(canonicalJson(event.provenance as unknown as CanonicalJsonValue)) !== row.provenance_hash) {
    ...
    if (classification !== null) {
      ...
      if (sha256Hex(marketEventClassificationJson(classification)) !== row.result_hash) throw new Error('Stored classification failed integrity validation.');
    }
    return { event, contentHash: row.content_hash, provenanceHash: row.provenance_hash,
      classification, classificationId: row.classification_id, classificationHash: row.result_hash };
  }));
}
