...
export function ChartExtensionManager({ client, onClose }: {
  ...
  const pickInstall = useCommand(client, 'chart-extensions.install.pick', INVALIDATE);
  ...
  return <div className="extension-manager-backdrop" role="presentation" onMouseDown={onClose}>
    <section ref={dialogRef} className="extension-manager" role="dialog" aria-modal="true" aria-labelledby="extensions-title" onMouseDown={(event) => event.stopPropagation()}>
      ...
      <section><h3>Install signed package</h3><button type="button" disabled={pickInstall.state.kind === 'pending'} onClick={() => void pickInstall.run({ commandId: crypto.randomUUID(), confirmed: true })}><FileUp size={14} /> Choose .coquichart package</button><details className="extension-advanced-import"><summary>Advanced: paste canonical package JSON</summary><label className="extension-field"><span>Canonical .coquichart JSON</span><textarea value={packageJson} onChange={(event) => setPackageJson(event.target.value)} placeholder='{"format":"coqui-chart-extension-v1",…}' /></label><button type="button" disabled={packageJson.trim().length === 0 || install.state.kind === 'pending'} onClick={() => void install.run({ commandId: crypto.randomUUID(), packageJson, confirmed: true })}>Verify signature and install</button></details></section>
