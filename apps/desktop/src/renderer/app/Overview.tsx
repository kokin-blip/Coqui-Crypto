...
export function Overview({ client }: { readonly client: CoquiClient }): React.JSX.Element {
  ...
  const recent = <section className="panel overview-recent"><header className="compact-panel-heading"><div><p className="eyebrow">Recent decisions</p><h2>Operational activity</h2></div></header><div className="compact-panel-body">{activity.kind === 'ready' && activity.value.events.length > 0 ? <ul className="overview-activity">{activity.value.events.map((event) => <li key={event.id}><strong>{event.title}</strong><span>{event.status} · {new Date(event.occurredAt).toISOString().slice(0, 10)}</span></li>)}</ul> : <div className="panel-empty-body"><p>No recorded decisions yet.</p></div>}</div></section>;
