...
import { formatApproxUsd, formatUsd, InteractivePanel } from '@coqui/ui-kit';
...
export function Overview({ client }: { readonly client: CoquiClient }): React.JSX.Element {
  ...
  const paperPerformanceExposures = portfolio.kind === 'ready' && portfolio.value !== null
    ...
    : [];
  const headlineValue = exploratoryPrimary ? exploratory.value!.currentEquityUsd :
    portfolio.kind === 'ready' && portfolio.value !== null ? portfolio.value.totalValueUsd : null;

  const decisionTitle = exploratoryPrimary ? 'Exploratory paper simulation is active'
    : risk.kind === 'ready' && (risk.value.blockReason !== null || risk.value.assessmentState === 'unassessed') ? 'Paper action is standing down on risk'
      : edgeStudy.kind === 'ready' && !edgeStudy.value.activated ? 'Forward edge is not verified'
        : edgeStudy.kind === 'ready' && edgeStudy.value.activated ? 'Review the current paper gate'
          : 'Checking paper decision evidence';
  const decisionDetail = exploratoryPrimary
    ? 'The current TrendVol strategy is unvalidated. Market freshness, safety controls, limits, fencing, and next-open settlement remain enforced.'
    : risk.kind === 'ready' && risk.value.blockReason !== null ? risk.value.blockReason
      : risk.kind === 'ready' && risk.value.assessmentState === 'unassessed' ? 'Sufficient paper-equity history is required before risk can be assessed.'
        : edgeStudy.kind === 'ready' && !edgeStudy.value.activated ? `${edgeStudy.value.completedDays} of 365 observed days and ${edgeStudy.value.costBearingRebalances} of 30 cost-bearing rebalances are recorded. No verified positive edge is active.`
          : 'Current evidence and risk gates are checked again before any paper submission.';
  const decision = <section className="decision-banner overview-decision" aria-labelledby="decision-summary-heading"><div><p className="section-label">Decision status</p><h2 id="decision-summary-heading">{decisionTitle}</h2><p>{decisionDetail}</p></div><a className="button-secondary" href={exploratoryPrimary ? '#/paper/overview' : risk.kind === 'ready' && (risk.value.blockReason !== null || risk.value.assessmentState === 'unassessed') ? '#/risk' : '#/research'}>{exploratoryPrimary ? 'Open paper simulation' : risk.kind === 'ready' && (risk.value.blockReason !== null || risk.value.assessmentState === 'unassessed') ? 'Inspect risk' : 'Inspect evidence'}</a></section>;
  const health = <section className="overview-health" aria-label="Portfolio and operating exposure">
    <article className="overview-number"><p className="eyebrow">{exploratoryPrimary ? 'Primary · paper simulation' : 'Connected portfolio'}</p><strong>{headlineValue === null ? 'Unavailable' : formatApproxUsd(headlineValue)}</strong><span>{exploratoryPrimary ? `Exploratory · unvalidated · ${exploratory.value!.counts.pending} pending` : portfolio.kind === 'ready' && portfolio.value !== null ? `${portfolio.value.exposures.length} exposures · ${portfolio.value.complete ? 'complete valuation' : 'valuation incomplete'}` : 'Sync Coinbase or Robinhood to populate'}</span>{headlineValue !== null && <details className="exact-value-detail"><summary>Exact value</summary>{formatUsd(headlineValue)?.text}</details>}</article>
