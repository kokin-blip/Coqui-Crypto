...
import { formatApproxUsd, formatQuantity, formatUsd, freshnessBadge } from '@coqui/ui-kit';
...
export function Portfolio({ client }: { readonly client: CoquiClient }): React.JSX.Element {
  ...
  if (section === 'connected') {
    ...
    const selectedExposure = connected.value.exposures.find((exposure) => exposure.exposureKey === selectedExposureKey) ?? connected.value.exposures[0];
    return <section aria-labelledby="portfolio-heading" className="space-y-4">{sectionHeader}
      <div className="connected-portfolio-summary"><div><p className="eyebrow">Authoritative connected total</p><strong>{connected.value.totalValueUsd === null ? 'Unavailable' : formatApproxUsd(connected.value.totalValueUsd)}</strong><small>{connected.value.complete ? 'Complete valuation' : 'Valuation incomplete — unavailable values are not treated as zero'}</small>{connected.value.totalValueUsd !== null && <details className="exact-value-detail"><summary>Exact connected total</summary><Money value={connected.value.totalValueUsd} /></details>}</div><p className="portfolio-as-of">As of <time dateTime={new Date(connected.value.asOfMs).toISOString()} title={new Date(connected.value.asOfMs).toISOString()}>{new Date(connected.value.asOfMs).toLocaleString()}</time> · {connected.value.connectionSnapshotIds.length} connection snapshot{connected.value.connectionSnapshotIds.length === 1 ? '' : 's'}</p></div>
      <div className="portfolio-master-detail"><div className="portfolio-table-scroll"><table className="w-full text-left"><caption className="sr-only">Authoritative connected holdings and account attribution</caption><thead><tr><th scope="col">Asset</th><th scope="col" className="text-right">Quantity</th><th scope="col" className="text-right">Value</th><th scope="col">Connection attribution</th></tr></thead><tbody>{connected.value.exposures.map((exposure) => <tr key={exposure.exposureKey} aria-selected={selectedExposure?.exposureKey === exposure.exposureKey}><th scope="row"><button type="button" className="holding-select" aria-pressed={selectedExposure?.exposureKey === exposure.exposureKey} onClick={() => setSelectedExposureKey(exposure.exposureKey)}>{exposure.exposureKey}</button></th><td className="text-right tabular-nums">{formatQuantity(exposure.quantity) ?? exposure.quantity}</td><td className="text-right tabular-nums"><Money value={exposure.valueUsd} /></td><td>{[...new Set(exposure.contributions.map((item) => item.provider === 'coinbase' ? 'Coinbase' : 'Robinhood Crypto'))].join(', ')}</td></tr>)}</tbody></table></div><aside className="holding-detail" aria-label="Connected holding detail">{selectedExposure === undefined ? <SurfaceState kind="empty" title="No connected holdings" compact /> : <><p className="section-label">Selected connected asset</p><h3>{selectedExposure.exposureKey}</h3><dl><div><dt>Quantity</dt><dd>{formatQuantity(selectedExposure.quantity) ?? selectedExposure.quantity}</dd></div><div><dt>Value</dt><dd><Money value={selectedExposure.valueUsd} /></dd></div><div><dt>Contributing accounts</dt><dd>{selectedExposure.contributions.length}</dd></div></dl><ul className="connected-contributions">{selectedExposure.contributions.map((item) => <li key={`${item.connectionId}:${item.accountRefId}:${item.instrument?.productId ?? 'asset'}`}><strong>{item.provider === 'coinbase' ? 'Coinbase' : 'Robinhood Crypto'}</strong><span>Account {item.accountRefId.slice(0, 8)}… · {item.instrument?.productId ?? selectedExposure.exposureKey}</span><small>{formatQuantity(item.quantity) ?? item.quantity} · <Money value={item.valueUsd} /></small></li>)}</ul></>}<div className="connected-allocation"><p className="section-label">Portfolio allocation</p><AllocationRing data={currentAllocation} selectedId={selectedExposure?.exposureKey ?? null} onSelect={setSelectedExposureKey} /></div></aside></div>
