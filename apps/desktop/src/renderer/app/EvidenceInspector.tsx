...
function InspectorContent({ client, presentation }: {
  readonly client: CoquiClient;
  readonly presentation: 'docked' | 'drawer';
}): React.JSX.Element {
  ...
  return (
    <aside className="evidence-inspector" style={presentation === 'docked' ? { width } : undefined} aria-label="Evidence inspector">
      <header><div><p className="eyebrow">Context</p><h2>Evidence inspector</h2></div><div className="inspector-actions">{presentation === 'docked' && <><button type="button" title="Narrow inspector" aria-label="Narrow evidence inspector" disabled={width <= 280 || workspace.pending} onClick={() => void workspace.update({ inspectorWidthPx: Math.max(280, width - 40) })}><Minus size={15} /></button><button type="button" title="Widen inspector" aria-label="Widen evidence inspector" disabled={width >= 420 || workspace.pending} onClick={() => void workspace.update({ inspectorWidthPx: Math.min(420, width + 40) })}><Plus size={15} /></button></>}<button type="button" title="Close inspector" aria-label="Close evidence inspector" onClick={workspace.closeInspector}><X size={16} /></button></div></header>
...
function InspectorDrawer({ client }: { readonly client: CoquiClient }): React.JSX.Element {
  const workspace = useWorkspace();
  const dialog = useRef<HTMLDivElement>(null);
  useDialogFocus(dialog, workspace.closeInspector);
  return (
    <div className="evidence-inspector-backdrop" role="presentation" onMouseDown={(event) => {
      if (event.target === event.currentTarget) workspace.closeInspector();
    }}>
      <div ref={dialog} className="evidence-inspector-dialog" role="dialog" aria-modal="true" aria-label="Evidence inspector" tabIndex={-1}>
        <InspectorContent client={client} presentation="drawer" />
      </div>
    </div>
  );
}

export function EvidenceInspector({ client, presentation }: {
  readonly client: CoquiClient;
  readonly presentation: 'docked' | 'drawer';
}): React.JSX.Element {
  return presentation === 'docked'
    ? <InspectorContent client={client} presentation="docked" />
    : <InspectorDrawer client={client} />;
}
