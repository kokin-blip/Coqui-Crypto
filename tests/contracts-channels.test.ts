...
describe('channel registry', () => {
  ...
  it('matches the boundaries that actually have tested services today', () => {
    expect([...CHANNEL_NAMES].sort()).toEqual([
      ...
      'accounts.workspace', 'accounts.workspace.set',
      'activity.feed',
      'advisor.chat.history', 'advisor.chat.history.delete', 'advisor.chat.history.export',
      'advisor.chat.send', 'advisor.context.prepare', 'advisor.decision.explain', 'advisor.facts.generate', 'advisor.navigation',
      'advisor.provider.connect', 'advisor.provider.disconnect', 'advisor.providers',
      'alerts.view',
      ...
      'market-data.yields',
      'market-events.ingest-local', 'market-events.timeline', 'operations.floor',
      'paper.campaign',
  ...
  it('classifies every channel, and names the one write', () => {
    ...
    expect(CHANNEL_KINDS.write).toEqual([
      ...
      'advisor.chat.send', 'advisor.decision.explain', 'advisor.facts.generate', 'advisor.navigation',
      'advisor.provider.connect', 'advisor.provider.disconnect',
      'chart-extensions.install',
      'chart-extensions.install.pick',
      'chart-extensions.remove',
      'chart-extensions.set',
